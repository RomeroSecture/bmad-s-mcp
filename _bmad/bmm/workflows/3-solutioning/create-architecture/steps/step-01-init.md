...
### 0. Secture Adaptation Gate (MANDATORY — execute BEFORE anything else)

🛑 **STOP. Before checking files, creating documents, or loading templates, you MUST execute your Secture Adaptation protocol.**

Execute Steps 1 and 2 of your `secture_adaptation` now:

1. **Emit the ARTIFACT INVENTORY** — scan for existing artifacts and report what you find.
...
The declared execution mode will govern the rest of this workflow:
- **GENERATE**: Execute all steps as designed — full creation from scratch.
- **REFINE**: Steps that say "create" or "generate" become "identify gaps and improve." Preserve existing valid content.
- **VERIFY**: Steps that say "create" or "generate" become "validate alignment." Do not produce new content.

Carry the declared mode through ALL subsequent steps. If a step instruction conflicts with the mode, adapt the instruction to the mode.
